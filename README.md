...
- Follow and unfollow controls are available in Explore and on public member profiles. Unfollow requires explicit confirmation. Successful new follows use a self-contained, full-screen VipKorner acknowledgement instead of a persistent status banner; block confirmation uses a branded in-app dialog instead of a browser prompt. For the current demo, every seeded or newly registered non-admin profile automatically follows the first active administrator.
