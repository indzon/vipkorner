...
- Feed avatars open the author profile and use the active-Short ring when that author has unseen Shorts. Members can follow unfollowed authors from a post header, hide individual posts from their own feed, double-tap media or use the heart to like with animated feedback and a stable optimistic count, bookmark posts into their Saved collection, and send posts through private messaging.
