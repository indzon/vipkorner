...
export async function PATCH(request: Request) {
  try {
    ...
    const showLocation = input.showLocation === undefined ? Number(Boolean(user.showLocation)) : Number(Boolean(input.showLocation));
    ...
    await DB.prepare(`UPDATE users SET username = ?, display_name = ?, bio = ?, website = ?, location = ?, show_location = ?,
      is_public = ?, story_replies = ?, high_quality_uploads = ?, saved_collection_public = ? WHERE id = ?`)
      .bind(username, displayName, bio, website, location, showLocation, isPublic, storyReplies, highQualityUploads, savedCollectionPublic, user.id).run();
