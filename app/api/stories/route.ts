...
export async function POST(request: Request) {
  try {
    ...
    if (request.headers.get("content-type")?.includes("application/json")) {
      ...
      if (input.action !== "react" || !id || !STORY_REACTIONS.has(emoji)) {
        return NextResponse.json({ error: "Choose an available short reaction." }, { status: 400 });
      }
      ...
      if (!story) return NextResponse.json({ error: "Short not found." }, { status: 404 });
      if (story.userId === user.id) return NextResponse.json({ error: "You cannot react to your own short." }, { status: 403 });
      if (!story.reactionsAllowed) return NextResponse.json({ error: "Short reactions are turned off for this member." }, { status: 403 });
      ...
      await DB.batch([
        ...
        DB.prepare("INSERT INTO notifications (id, user_id, actor_id, type, entity_id, message, created_at) VALUES (?, ?, ?, 'story_reaction', ?, ?, ?)")
          .bind(crypto.randomUUID(), story.userId, user.id, id, `@${user.username} reacted ${emoji} to your short.`, now),
      ]);
      const count = await DB.prepare("SELECT COUNT(*) AS total FROM story_reactions WHERE story_id = ?").bind(id).first<{ total: number }>();
    ...
    if (!(media instanceof File) || !upload) return NextResponse.json({ error: "Choose a photo or video for your short." }, { status: 400 });
  ...
  } catch (error) { return authErrorResponse(error) || NextResponse.json({ error: "Could not share this short." }, { status: 500 }); }
}
...
export async function PUT(request: Request) {
  try {
    ...
    const id = String(input.id || "");
    if (!id) return NextResponse.json({ error: "Short id is required." }, { status: 400 });
    const { DB } = bindings();
  ...
  } catch (error) { return authErrorResponse(error) || NextResponse.json({ error: "Could not mark this short as viewed." }, { status: 500 }); }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!story) return NextResponse.json({ error: "Short not found." }, { status: 404 });
    if (story.userId !== user.id) return NextResponse.json({ error: "Only the short owner can delete it." }, { status: 403 });
    await DB.batch([
  ...
  } catch (error) { return authErrorResponse(error) || NextResponse.json({ error: "Could not delete this short." }, { status: 500 }); }
}
