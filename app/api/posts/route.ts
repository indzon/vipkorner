...
export async function POST(request: Request) {
  ...
  const upload = image instanceof File ? await inspectMediaUpload(image) : null;
  const isImage = upload?.kind === "image";
  const isVideo = upload?.kind === "video";
  if (!(image instanceof File) || !upload) {
  ...
  await MEDIA.put(key, image.stream(), { httpMetadata: { contentType: upload.contentType } });
...
export async function PUT(request: Request) {
  ...
  if (!id || !(media instanceof File) || !upload) {
