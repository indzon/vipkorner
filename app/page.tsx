...
export default function HomePage() {
  ...
  async function hidePost(postId: string) {
    ...
    if (!hiddenPost || hiddenPost.owned) return;
    setPosts((current) => current.filter((post) => post.id !== postId));
    try {
      const response = await fetch("/api/posts", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: postId, action: "hide" }) });
      await readApiResponse(response, "Could not hide this post.");
      setToast("Post hidden from your feed.");
    } catch (reason) {
      setPosts((current) => current.some((post) => post.id === postId) ? current : [...current, hiddenPost].sort((a, b) => b.createdAt - a.createdAt));
      throw reason;
    }
  }
...
function PostCard({ post, profile, hasUnseenShorts, onViewProfile, onFollow, onToggle, onComment, onCaptionUpdate, onDelete, onHide }: { post: Post; profile: Profile; hasUnseenShorts: boolean; onViewProfile: (userId: string) => void; onFollow: (author: PublicUser) => Promise<{ following?: boolean; requested?: boolean }>; onToggle: (id: string, action: "like" | "save", desired?: boolean) => void; onComment: (postId: string, body: string) => Promise<void>; onCaptionUpdate: (postId: string, caption: string) => Promise<void>; onDelete: (postId: string) => Promise<void>; onHide: (postId: string) => Promise<void> }) {
  ...
  return (
    <article className="post-card">
      <header className="post-header"><div className="post-author"><button type="button" className={`post-author-avatar ${hasUnseenShorts ? "has-unseen-short" : ""}`} onClick={() => onViewProfile(post.author.id)} aria-label={`View @${post.author.username}'s profile`}><img src={profileImage(post.author)} alt="" /></button><button type="button" className="post-author-name" onClick={() => onViewProfile(post.author.id)}><strong>{post.author.username}</strong><time className="post-header-time" dateTime={new Date(post.createdAt).toISOString()}>{relativeTime(post.createdAt)}</time></button></div><div className="post-header-actions">{!post.owned && !post.author.following && <button type="button" className={`post-follow-button ${post.author.followRequestStatus === "pending" ? "following" : ""}`} disabled={followBusy} onClick={() => void toggleFollow()}>{followBusy ? "…" : post.author.followRequestStatus === "pending" ? "Requested" : "Follow"}</button>}<div className="post-menu-wrap"><button className="icon-button" aria-label="Post options" aria-expanded={optionsOpen} onClick={() => setOptionsOpen((open) => !open)}><MoreHorizontal /></button>{optionsOpen && <div className="post-menu">{post.owned && <button onClick={() => { setCaptionDraft(post.caption); setEditingCaption(true); setOptionsOpen(false); }}>Edit caption</button>}<button onClick={async () => { await navigator.clipboard?.writeText(`${location.origin}/?post=${post.id}`); setOptionsOpen(false); }}>Copy post link</button>{!post.owned && <button className="post-menu-hide" onClick={async () => { setOptionsOpen(false); try { await onHide(post.id); } catch (reason) { setActionError(reason instanceof Error ? reason.message : "Could not hide post."); } }}><X /> <span>Hide post</span></button>}{post.owned ? <button className="post-menu-danger" onClick={async () => { if (!window.confirm("Delete this post permanently?")) return; setOptionsOpen(false); try { await onDelete(post.id); } catch (reason) { setActionError(reason instanceof Error ? reason.message : "Could not delete post."); } }}>Delete post</button> : <button className="post-menu-danger" onClick={() => { setOptionsOpen(false); setReportOpen(true); }}><Flag /> <span>Report post</span></button>}<button onClick={() => setOptionsOpen(false)}>Cancel</button></div>}</div></div></header>
  ...
  return (
    <div className="media-viewer" role="dialog" aria-modal="true" aria-label="Post media viewer" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      ...
      <section className={`media-viewer-card ${activeMedia.mediaType === "video" ? "video-viewer-card" : "image-viewer-card"}`}>
        <div className="media-viewer-stage">{activeMedia.mediaType === "video" ? <><video ref={videoRef} key={activeMedia.id} src={imageSource(activeMedia)} autoPlay muted={videoMuted} controls playsInline preload="auto" onCanPlay={(event) => event.currentTarget.play().catch(() => undefined)} /><button type="button" className="media-audio-toggle" onClick={toggleViewerSound} aria-label={videoMuted ? "Unmute video" : "Mute video"}>{videoMuted ? <VolumeX /> : <Volume2 />}</button></> : <img key={activeMedia.id} src={imageSource(activeMedia)} alt={activeMedia.caption || post.caption} />}{mediaItems.length > 1 && <><button type="button" className="carousel-arrow carousel-previous" onClick={() => showMedia((activeMediaIndex - 1 + mediaItems.length) % mediaItems.length)} aria-label="Previous carousel item"><ChevronLeft /></button><button type="button" className="carousel-arrow carousel-next" onClick={() => showMedia((activeMediaIndex + 1) % mediaItems.length)} aria-label="Next carousel item"><ChevronRight /></button><span className="carousel-count">{activeMediaIndex + 1}/{mediaItems.length}</span></>}</div>
